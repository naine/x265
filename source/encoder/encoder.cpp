...
void Encoder::printSummary()
{
    ...
    for (int layer = 0; layer < m_param->numLayers; layer++)
    {
        ...
        {
            int p = 0;
            int64_t elapsed_us = x265_mdate() - m_encodeStartTime;
            ...
            double bitrate = (0.001 * m_analyzeAll[layer].m_accBits) / elapsedVideoTime;

            p += snprintf(buffer + p, sizeof(buffer) - p, "\nencoded %d frames in %d:%02d:%02d.%02d (%.2f fps), %.2f kb/s, Avg QP:%2.2lf", m_analyzeAll[layer].m_numPics,
